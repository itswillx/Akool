...
// Design tokens for the app's dense data surfaces — the finance panel, its
// submodules and the documents "Rede" section. Maps the "Controle Financeiro"
// design onto the site's CSS variables so every module keeps the same contrast
// (decision: graphite accent, income green, expense red). Use these instead of
// hardcoded hexes.
//
// The FIN_ prefix is historical (these lived in modules/finance/ui until the
// documents graph needed them). They are semantic aliases of --color-done /
// --color-error / --color-btn-primary, not finance-only values; renaming would
// ripple through ten files for nothing.
