...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // Fica ANTES do ramo `action === "test"` de proposito -- o test tambem bate
    // na API do provider (testApiKey), entao deixa-lo fora do contador criaria
    // um caminho livre. check_rate_limit tem EXECUTE so para service_role e NAO
    // da raise: devolve jsonb e o 429 e montado aqui. Ver
    // supabase/migrations/20260812170000_sec_rate_limit_core.sql.
    const rate = await checkRateLimit(serviceClient, "photo_analyze:uid", user.id, 30, 3600);
